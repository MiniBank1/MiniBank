...
public class AuthService : IAuthService
{
    ...
    public async Task<(bool Success, string Message)> VerifyRegisterAsync(VerifyRegisterDto dto)
    {
        ...
        try
        {
            ...
            var cardResult = await _cardService.CreateCardForAccountAsync(currentAccount.AccountId);
            if (!cardResult.Success)
            {
                await transaction.RollbackAsync();
                return (false, "User created, but current account card could not be created.");
            }

            await _notificationService.CreateAsync(
                user.UserId,
                "Your current account has been created and your virtual card is ready."
            );
        ...
        {
            await transaction.RollbackAsync();
            return (false, "An unexpected error occurred during registration verification.");
        }
    }
